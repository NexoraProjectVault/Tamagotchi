...
def current_user_id():
    ...
    if not uid: abort(401, "missing user id")
    return uid
...
def create_task():
    ...
    print(f'Data received for creating a new task: {data}')
    ...
    due_at = parse_iso(data.get("due_at"))  # 2025-11-08 00:00:00
    if due_at:
        due_at = due_at.replace(hour=23, minute=59, second=59)  # set to end of day
        if due_at <= _utc_now():
            abort(400, "due_at must be in the future (UTC)")
    else:
        due_at = _utc_now().replace(hour=23, minute=59, second=59)
...
def list_tasks():
    ...
    if tag:
        ...
        try:
            ...
            q = q.filter(or_(
                ...
                func.cast(Task.tags, db.Text).like(like)
            ))
    ...
    if due_before:
        q = q.filter(Task.due_at.isnot(None), Task.due_at <= datetime.fromisoformat(due_before))
    ...
    def apply_sort(q, key):
        ...
        if col in mapping:
            column = mapping[col]
            return q.order_by(nulls_last(direction(column)))
      
        if col == "tag":
            return q.order_by(direction(text("tags::text")))
        return q
...
def complete_task(task_id):
    ...
    if t.completed_event_emitted_at is None:
        ...
        try:
            r = requests.post(f"{PET_SERVICE_URL}/events", json=payload, timeout=3)
            if r.ok:
